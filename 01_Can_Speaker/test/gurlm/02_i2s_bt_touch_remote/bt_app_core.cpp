...
{
    Serial.println("bt_app_work_dispatchevent 0x");
...
static bool bt_app_send_msg(bt_app_msg_t *msg)
{
    ...
    if (xQueueSend(bt_app_task_queue, msg, 10 / portTICK_RATE_MS) != pdTRUE) {
        Serial.println("bt_app_send_msg xQueue send failed");
        return false;
    }
    return true;
}
...
static void bt_app_task_handler(void *arg)
{
    ...
    for (;;) {
        if (pdTRUE == xQueueReceive(bt_app_task_queue, &msg, (portTickType)portMAX_DELAY)) {
            Serial.println("bt_app_task_handler, sig 0x");
            switch (msg.sig) {
            case BT_APP_SIG_WORK_DISPATCH:
                bt_app_work_dispatched(&msg);
                break;
            default:
                Serial.println("%s, unhandled sig:");
                break;
            } // switch (msg.sig)
